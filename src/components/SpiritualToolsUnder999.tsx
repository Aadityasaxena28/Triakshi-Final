...
const SpiritualToolsUnder999 = () => {
  ...
  if (isLoading) {
    return (
      <section className="py-4 bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif' }}>
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            <div className="animate-spin rounded-full h-8 w-8 border-b-2 border-amber-600 mx-auto"></div>
            <p className="mt-2 text-gray-600 text-xs">Loading spiritual treasures...</p>
          </div>
        </div>
      </section>
    );
  ...
  return (
    <section className="py-4 bg-gradient-to-br from-amber-50 via-orange-50 to-yellow-50 relative overflow-hidden" style={{ fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", Roboto, "Helvetica Neue", Arial, sans-serif' }}>
      {/* Decorative elements */}
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        ...
        <div className="text-center mb-3">
          <div className="flex items-center justify-center gap-1.5 mb-1">
            ...
            <h2 className="text-xl sm:text-3xl font-bold relative inline-block overflow-hidden">
            ...
            <Sparkles className="w-4 h-4 text-amber-600 animate-pulse" style={{ animationDelay: '0.5s' }} />
          </div>
          
          <p className="text-gray-700 text-xs font-medium max-w-2xl mx-auto flex items-center justify-center gap-1.5">
          ...
          <div className="mt-2 inline-flex items-center gap-1 bg-gradient-to-r from-amber-600 to-orange-600 text-white px-3 py-1 rounded-full text-[10px] font-semibold shadow-lg">
        ...
        <div className="flex justify-center md:justify-end mb-2 gap-1.5">
          <Button
            ...
            size="icon"
            onClick={prevSlide}
            className="rounded-full h-7 w-7 bg-white/80 backdrop-blur-sm border-amber-300 hover:bg-amber-100 hover:border-amber-400 shadow-lg transition-all"
          >
        ...
        >
          {products.map((product: Product) => {
            ...
            return (
              <div
                key={product.id}
                className="min-w-[160px] max-w-[160px] flex-shrink-0 bg-white border-2 border-amber-200 rounded-xl shadow-xl hover:shadow-2xl transition-all duration-300 cursor-pointer hover:-translate-y-2 relative overflow-hidden"
                style={{
              ...
              >
                {/* Discount Badge */}
                {discount > 0 && (
                  <div className="absolute top-1.5 left-1.5 bg-gradient-to-r from-amber-500 to-orange-500 text-white text-[10px] font-bold px-2 py-0.5 rounded-full shadow-lg z-10 animate-pulse">
                    {discount}% OFF
                  </div>
                )}
                ...
                {/* Under 999 Badge */}
                <div className="absolute top-7 right-1.5 bg-gradient-to-r from-purple-500 to-pink-500 text-white text-[9px] font-bold px-1.5 py-0.5 rounded-full shadow-lg z-10">
                  Under ₹999
                </div>

                {/* Festive corner decoration */}
                <div className="absolute top-0 right-0 w-10 h-10 bg-gradient-to-br from-yellow-400 to-amber-400 opacity-20 rounded-bl-full"></div>
                ...
                >
                  ...
                  <div className="absolute inset-0 bg-black/40 opacity-0 hover:opacity-100 transition-opacity duration-300 flex items-center justify-center">
                    <Button
                      variant="secondary"
                      size="sm"
                      className="bg-white text-amber-600 hover:bg-amber-50 text-[10px] h-6 px-2"
                      onClick={(e) => {
                        e.stopPropagation();
                ...
                <div className="p-2">
                  <h3 
                    className="text-xs font-semibold mb-1 text-gray-800 hover:text-amber-600 transition-colors line-clamp-2 cursor-pointer leading-tight"
                    onClick={() => handleViewDetails(product)}
                  >
                  ...
                  {product.category && (
                    <div className="mb-1">
                      <span className="text-[9px] bg-amber-100 text-amber-700 px-1.5 py-0.5 rounded-full font-semibold capitalize">
                        {product.category}
                      </span>
                    </div>
                  )}
        ...
        <div className="mt-3 flex justify-center">
          <div className="w-full max-w-md">
            <div className="h-1 bg-amber-200 rounded-full overflow-hidden shadow-inner">
              <div 
